...
public class GeradorAleatorioDeXML {
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		BigDecimal valor = BigDecimal.valueOf(40d);
		int quantidade = 1000;
		...
		for (int dias = 0; dias < 30; dias++) {
			...
			for (int negociacao = 0; negociacao < quantidadeNegociacoesDoDia; negociacao++) {
				...
				valor = valor.add(BigDecimal.valueOf((random.nextInt(200)-100)/100.0));

				if (valor.doubleValue() < 5.0 ) {
					valor = BigDecimal.valueOf(5.0);
				}
